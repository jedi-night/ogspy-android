...
public class DialogAccount extends DialogActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final DatabaseAccountHandler accountHandler = OgspyActivity.getHandlerAccount();
        ...
        if (ACCOUNT_NEW.equals(creation)) {
            ...
            buttonDelete.setOnClickListener(
                    v -> DialogHandler.confirm(activity, "Suppression de compte", "Voulez-vous réellement supprimer le compte ?", "Annuler", "Ok",
        ...
        buttonSave.setOnClickListener(v -> {
            if (OgspyUtils.checkUserAccount(user.getText().toString(), password.getText().toString(), serverOgspy.getText().toString())) {
                ...
                DownloadServerConnection serverConnection = new DownloadServerConnection(account, creation);
                DownloadTask.executeDownload(serverConnection);
            }
        });
    ...
    private void deleteAccount(DatabaseAccountHandler accountHandler, String accountId) {
        if (accountHandler.deleteAccountById(accountId) != -1) {
            CommonUtilities.displayMessage(this, "Le compte a été supprimé");
            ...
            new DialogHandler().showException(this, new OgspyException("Le compte n'a pu être supprimé", Constants.EXCEPTION_DATA_DELETE));
        }
        finish();
    }

    private String getServerUrlFromSelectedPosition(int positionSelected) {
        return getResources().getStringArray(R.array.servers_values)[positionSelected];
    }
