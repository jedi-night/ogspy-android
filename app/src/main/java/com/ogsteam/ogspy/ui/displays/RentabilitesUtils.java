...
public abstract class RentabilitesUtils {
    ...
    public static void showRentabilites(RentabilitesHelper helperRentabilites, String type) {
        ...
        if (helperRentabilites != null && helperRentabilites.getRentabilites() != null && helperRentabilites.getRentabilites().size() > 0) {
            ...
            if (Constants.RENTA_TYPE_ME.equals(type) && helperRentabilites.getRentabilites().containsKey(account.getUsername())) {
                ...
                setRentabiliteText(Float.parseFloat(String.valueOf(categories.getValue(0) + categories.getValue(1) + categories.getValue(2))));
            }
                ...
                setRentabiliteText(rentaGlobale);
            } else {
                setRentabiliteText(0);
            }
        } else {
            setRentabiliteText(0);
        }
        RelativeLayout piChartContainer = RentabilitesFragment.getPieChartContainer();
        if (categories.getItemCount() > 0) {
            GraphicalView piechart = ChartFactory.getPieChartView(OgspyApplication.getContext(), categories, getDefaultRenderer(categories.getItemCount()));
            if (Constants.RENTA_TYPE_MEMBER.equals(type)) {
                addPieOnClickListener(OgspyApplication.getApplication(), helperRenta, piechart);
            }
    ...
    private static void setRentabiliteText(float value) {
        if (RentabilitesFragment.getRentabilityTotal() != null) {
            RentabilitesFragment.getRentabilityTotal().setText("Rentabilité : " + NumberUtils.format(value));
        }
    }

    private static void addPieOnClickListener(final Context activity, final RentabilitesHelper helperRenta, GraphicalView piechart) {
